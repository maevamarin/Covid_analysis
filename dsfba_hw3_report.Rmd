...
geom_line(aes(x=date, y=Deaths, col="Deaths")) +
  ...
  labs(title = "Higly increase evolution for the confirmed case",
...
```{r fig-3, fig.cap="Figure 3", out.width="80%", fig.asp=.75,fig.align='center'}
 covid19_data %>%
  ...
  ggplot(aes(x=date, y= Confirmed, colour= region)) +
  geom_line() +
   labs(title = "Flat increase and early deperature in confirmed case in the East Asia & Pacific",        subtitle = "Worldwide representation group by region",
        x= "Date",
        y= "Confirmed case",
        caption = "data from jh_covid19_data_2020-04-06.csv") +
 scale_x_date(date_breaks = "1 week", date_labels = "%b %d" ) +
...
```{r fig-4, fig.cap="Figure 4", out.width="80%", fig.asp=.75,fig.align='center'}
 ...
 covid19_data %>%
  ...
  ggplot(aes(x=date, y= Deaths, colour= region)) +
  geom_line() +
 labs(
   title = "Exponential curve for the deaths in the region of Europe \n& Central Asia",
   subtitle = "Worldwide representation group by region",
   x= "Date",
   y= "Deaths",
   caption = "data from jh_covid19_data_2020-04-06.csv") +
   scale_x_date(date_breaks = "1 week", date_labels = "%b %d" ) +
...
```{r fig-5, fig.cap="Figure 5", out.width="80%", fig.asp=.75,fig.align='center'}
  ...
  geom_line()+
  labs(
    title = "We observe an increase in term of the ratio  deaths/confirmed ",
    subtitle = "Worldwide representation groupy by region",
    x= "Date",
    y= "Deaths/Confirmed",
    caption = "data from jh_covid19_data_2020-04-06.csv") +
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d" ) +
...
covid19_data %>%
  ...
  geom_line()+
  labs(
    title = "Europe & Central Asia \nhas the higest confirmed case by the population",
    subtitle = "Worldwide representation groupy by region",
    x= "Date",
    y= "Confirmed /Population",
   caption = "data from jh_covid19_data_2020-04-06.csv") +
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d" ) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r fig-7, fig.cap="Figure 7", out.width="80%", fig.asp=.75,fig.align='center'}
 ...
 covid19_data <- covid19_data %>% mutate(ratio_death_pop=(deaths/population))
covid19_data$ratio[is.nan(covid19_data$ratio_death_pop)]<-0

covid19_data %>%
  group_by(date,region) %>%
  summarize(Mean_ratio_death_pop = mean(ratio_death_pop)) %>%
  ggplot(aes(x=date, y= Mean_ratio_death_pop, colour= region)) +
  geom_line()+
  labs(
    title = "Europe & Central Asia \nhas the higest deaths ratio by the population",
    subtitle = "Worldwide representation groupy by region",
    x= "Date",
    y= "Deaths /Population",
   caption = "data from jh_covid19_data_2020-04-06.csv") +
...
covid19_data_filtered %>% 
  ...
  ggplot(aes(x=date,y=confirmed, fill=country, col=country)) +
  geom_line()+
  labs (
         y= "Confrimed case",
...
covid19_data_filtered %>% 
  filter(confirmed>=50000)%>%
  ggplot(aes(x=date,y=confirmed_1e5pop, fill=country, col=country))
+geom_line()+
  labs (
